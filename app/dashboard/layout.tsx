...
import { LayoutDashboard, Users, Zap, User, LogOut, Menu, UserCog, Shield } from 'lucide-react';
...
export default function DashboardLayout({
    ...
    const menuItems = [
        ...
        { name: 'Connectors', href: '/dashboard/connectors', icon: Zap },
    ...
    return (
        <div className="flex flex-col md:flex-row h-[100dvh] bg-background text-foreground overflow-hidden">
            {/* Desktop Sidebar */}
            <aside className="w-64 border-r border-border bg-card/50 hidden md:flex flex-col h-full flex-shrink-0">
                <SidebarNav />
            </aside>
            ...
            <div className="md:hidden flex items-center justify-between border-b border-border px-4 py-3 bg-card/50 flex-shrink-0">
                <h2 className="text-xl font-bold text-primary">Axerra</h2>
                <Button
                    variant="ghost"
                    size="sm"
